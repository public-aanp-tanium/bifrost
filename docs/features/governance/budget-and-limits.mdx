...
- **Window opened before its most recent boundary** - it is **already due** under the new rule, so the next reset sweep resets it and **clears its current usage**: spend for a budget, the token or request counter for a rate limit. For a monthly window switched on partway through a month, this is the common case.
